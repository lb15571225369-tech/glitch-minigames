...
Drilling.Update = function(callback)
    ...
    if not gameplayOnlyMode and cameraHandle then
        DestroyCam(cameraHandle, false)
...
exports('StartDrillingGameplayOnly', function()
    if Drilling.Active then
        return false
    end

    local p = promise.new()
    gameplayOnlyMode = true

    Drilling.Start(function(success)
        gameplayOnlyMode = false
        if type(success) ~= "boolean" then
            success = false
        end
        p:resolve(success)
